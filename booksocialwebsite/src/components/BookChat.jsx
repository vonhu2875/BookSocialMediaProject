...
export default function BookChat({ bookId }) {
  const HISTORY_PAGE_SIZE = 2;
  const quickPrompts = [
    'Tóm tắt nội dung chính của cuốn sách này bằng 3 điểm',
    'Những nhân vật nào được nhắc đến trong sách?',
    'Cuốn sách này nói về chủ đề gì?',
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      ...
      try {
        const firstPageResponse = await bookService.getBookChatHistory(bookId, 0, HISTORY_PAGE_SIZE);
        const lastPage = Math.max((firstPageResponse?.totalPages || 1) - 1, 0);
        const response = lastPage === 0
          ? firstPageResponse
          : await bookService.getBookChatHistory(bookId, lastPage, HISTORY_PAGE_SIZE);
        ...
        console.error('Lỗi lấy lịch sử hỏi đáp sách:', error);
      } finally {
        if (isCurrentBook) setLoadingHistory(false);
      }
    };
  ...
  const loadOlderMessages = async () => {
    if (!bookId || !hasOlderMessages || loadingOlder || isLoadingOlderRef.current) return;
    ...
    try {
      const response = await bookService.getBookChatHistory(bookId, nextPage, HISTORY_PAGE_SIZE);
      const history = Array.isArray(response?.content) ? response.content : [];
      ...
      console.error('Lỗi tải thêm lịch sử hỏi đáp sách:', error);
    } finally {
  ...
  const handleDeleteChatHistory = async () => {
    ...
    const confirmed = window.confirm('Bạn có chắc chắn muốn xóa toàn bộ lịch sử hỏi đáp của cuốn sách này không?');
    if (!confirmed) return;

    setDeletingHistory(true);

    try {
      await bookService.deleteBookChatHistory(bookId);
      setMessages([]);
      setHistoryPage(0);
      setHasOlderMessages(false);
    } catch (error) {
      console.error('Lỗi xóa lịch sử hỏi đáp sách:', error);
    } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!trimmedQuestion || sending || !bookId) return;
    ...
    try {
      const response = await bookService.chatBook(bookId, {
      ...
      setMessages((currentMessages) => [
        ...currentMessages,
        {
          id: `answer-${Date.now()}`,
          role: 'assistant',
          content: answer || 'AI chưa tìm thấy câu trả lời phù hợp trong cuốn sách này.',
        },
      ]);
    } catch (error) {
      console.error('Lỗi hỏi đáp sách:', error);
      setMessages((currentMessages) => [
        ...currentMessages,
        {
          id: `error-${Date.now()}`,
  ...
  return (
    <>
      <div
        aria-hidden={!isOpen}
        className={`fixed bottom-4 right-20 z-50 flex h-[min(560px,calc(100vh-7rem))] w-[calc(100%-1.5rem)] max-w-[360px] flex-col rounded-2xl border border-indigo-500/30 bg-slate-900 shadow-2xl shadow-black/50 transition-all duration-300 ease-out ${
          isOpen
      ...
      >
        <div className="flex h-full min-h-0 w-full flex-col overflow-hidden">
          <div className="flex shrink-0 items-center justify-between border-b border-slate-800 p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400">Hỏi đáp về cuốn sách</p>
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="flex h-full min-h-0 flex-1 flex-col gap-2 p-4 overflow-hidden">
                ...
                <>
                  ...
                  {messages.length === 0 && (
                    <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-950/40 px-4 py-5 text-center text-xs leading-relaxed text-slate-500">
                      Đặt câu hỏi về nội dung, nhân vật, chủ đề hoặc ý chính của cuốn sách này.
                    </div>
                  )}
            ...
            <form onSubmit={handleSubmit} className="flex items-center gap-2 border-t border-slate-800 pt-3">
              ...
              <input
                ...
                placeholder="Bạn muốn biết điều gì về cuốn sách?"
                disabled={sending}
                aria-label="Câu hỏi về cuốn sách"
